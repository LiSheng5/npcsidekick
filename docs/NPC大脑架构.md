...
> 最后更新：2026-09-18（§23 增 GTA5 内容移出仓库标注；§6 测试基线更新；世界声明键总表另见 `docs/世界声明键.md`）。原 2026-09-15 条目为新增 §29 能力审计结论 / §30 演进路线图。记录 NPC 层（`npc/` 目录）的完整架构，覆盖：三角色决策、落账口、记忆系统三阶段（反思/关联/遗忘）、防篡改+沙箱分级、审批策略配置化、语音系统、模型可插拔、上下文压缩、Codex 设计吸收（§9）、六项强化：动态资源词典/中文分词/多世界隔离/SSE 推送/清单解析/接入 SDK（§14）、调试台三件套（§15）、**Runtime 能力边界与三条断层（§29）**、**演进路线图与待拍板（§30）**。
...
> 本节原记录当时的协作流程与「未决清单」，属**内部商议内容**，已于 2026-09-19 移出公开仓库
> （归档于仓库外的 `_协作文档/`）。
> 相关机制见 §19（LLM 调度队列）与 §21（网关抖动重试）。
> **章节号保留不重排** —— 全文多处交叉引用后续编号。
...
1. ~~**记忆只进对话，不进决策**~~ → **2026-09-16 起部分合拢** —— 原状：记忆卡 / 反思 / 画像 / 管家约占
   `npc/` 近半代码，对**自主行为影响为 0**（`scheduler.py` 全文不 import `memory`），闭环断在"行动"这一环。
   **现状**：① **目标**（`NPC_GOALS`，人设 `goals` 降为种子）与 ② **结构化反思 lesson**
   （`NPC_LESSONS`，条目带可选 `scope`+`recommendation`）**都已进 `_dynamic_weight` 调权**，
   且各有 A/B 证据（`scripts/probe_lesson_ab.py`：有"避着砍木头"的经验 → 选该活的次数 **157 → 98，降 38%**）。
   **仍未进决策**：任意检索到的记忆、画像（persona 文件）、管家产出 —— 目前只有**带 scope 的反思条目**参与。
   另注：**P-6（2026-09-16）打通的是反方向**（目标 → 记忆召回，`NPC_GOAL_RELEVANCE`），不改变"记忆还没进决策"这个事实。
...
|---|---|---|
| **G1** | ~~决策层读记忆/目标~~ ✅ **已落地（2026-09-16）** | `npc/scheduler.py`：`goals_enabled()`（开关 `NPC_GOALS`，默认关）+ `npc_goals()`（惰性从人设播种 / 现读刷新）+ `_goal_factor()`（按优先级抬权 9→×3 · 5→×1 · 1→×1/3）+ `_goal_candidates()`（目标能补出日常表里没有的活）+ `_advance_goals_on_complete()`（整链干完 → 目标 +1）；`/api/version` 的 `features.flags.goals` 暴露生效态；锚 `tests/test_goal_decide.py` 16 例。✅ **目标 + 结构化反思 lesson 均已进决策**（`NPC_GOALS` / `NPC_LESSONS`）；仍未进：任意检索记忆 / 画像 / 管家产出（见 §30.1 第 5 步） |
...
| # | 债 | 性质 | 一句话 |
|---|---|---|---|
| TD1 | ~~记忆不进决策~~ **部分合拢（2026-09-16）** | 架构级断层 → 已修一半 | 见 29.1-1：**目标**（`NPC_GOALS`）与**结构化反思 lesson**（`NPC_LESSONS`）已进决策；**任意检索记忆 / 画像 / 管家产出仍未进** |
...
> 本节原为内部卫生记录，已于 2026-09-19 移出公开仓库（归档于仓库外 `_协作文档/`）。
> 仅保留一条正文所需的说明：**正文中的「任务书#0X」指 2026-08 的内部执行指令文件，
> 不在本仓库中** —— 其成果见 §23~§28。
...
**参考世界已声明 `"game"`（2026-09-17）**：`default_world()` 现在声明 `"_autonomy": "game"` ——
理由：跑起服务却没人连时不该空烧 token（"1 真小时 = 20 游戏小时"）。
**观察能力不受影响**：Console 本身就是客户端（2 秒轮询），打开就自动恢复推进。
**要开元宇宙模式**（玩家退出后世界继续演化）→ 把那条声明改成 `"free"`。
（`npc/npc.py:86` 的 `NPC()` 与 `server.load_village()` 都走 `default_world()` → 参考世界行为一致。）
