...
**可观测**（10 个开关暴露生效态，见 `/api/version` 的 `features.flags`；Web Console 7 页全可达）。
...
| R2 | **10 个** `NPC_*` 功能开关默认 OFF 的"暗路径"（2026-09-16 起 +`NPC_GOALS` / `NPC_LESSONS`） | benchmark 必须显式声明开关矩阵；**开关打开态也要跑一遍测试**（G1 正是靠这条抓到 `routine: None` 的真 bug） |
