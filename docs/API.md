...
- 资源不足 → 诚实拒绝（**参考世界**措辞："……木材现在弄不到了，采空了，等它长回来吧。"，由 `_review_tpl.exhausted` 声明；引擎中性默认："……{resource}现在弄不到了。"—— 不替任何游戏断言资源会再生）
...
> **权威源 = 代码**：`npc/server.py`（28 条）+ `npc/console_api.py`（14 条）—— 本表与代码不一致时，以代码为准。

### 9.1 游戏面（`npc/server.py`，28 条）
