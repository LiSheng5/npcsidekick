"""
NPCSidekick — Agent B 任务编译 + 规则层审查。

模块化分工（2026-08-21 架构决策）:
  B1 对话  — LLM 生成回复文本（表象）
  B2 编译  — 把玩家诉求编译成任务单（唯一落账口）
  A  审查  — 【**已退役 · §22 / 2026-08-25**】原为 LLM 语义深审，现由规则层安全闸承担；
             机器封存于 `npc/npc.py::_a_semantic_block`，`NPC_SUBAGENT_A` 读到仅告警忽略
             （与 `npc/subagent.py` 头注同款说明 —— T-08 之前两处不同步）

铁律（论文《The LLM Proposes, the Executive Disposes》+ 项目实测）:
  - 承诺成立 = 指令通过规则层安全闸并被 B2 推进活动队列，不是嘴上那番话
